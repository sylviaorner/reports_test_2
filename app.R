...
ui <-fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      checkboxGroupInput("libs", "Select your Library or Libraries",choices=NULL)
    ),
    mainPanel(
      h3("E1. Number of Items in the library's catalog as of December 31, 2024"),
      downloadButton("download_totalitems", "Download CSV"),
      ...
      h3("F3. Circulation of physical books, physical audio units, physical video units, and physical periodicals/serials"),
...
server <-function(input, output, session){
  ...
  #Helper function for creating .csv tables. 
  makeDownloadHandler <- function(outputId, filenamePrefix, dataFunc) {
    output[[outputId]] <- downloadHandler(
      filename = function() {
        paste0(filenamePrefix, "_", Sys.Date(), ".csv")
      },
      content = function(file) {
        # Always write out what dataFunc() produces
        write.csv(dataFunc(), file, row.names = FALSE)
      }
    )
  }
...
#Populate selected libraries
  ...
  observeEvent(input$org, {
    ...
    libs <- sort(libs)
    updateCheckboxGroupInput(session, "libs", choices = libs)
  })
  observeEvent(input$select_all, {
    libs <- unique(df[df$Parent.Org == input$org, "Owning.Library"])
    updateCheckboxGroupInput(session, "libs", selected = libs)
  })
  
  observeEvent(input$clear_all, {
    updateCheckboxGroupInput(session, "libs", selected = character(0))
  })
...
#Adding data for E3 using Owning Library and Circulating Library as matchpoint. 
  filtered_e3<-reactive ({
    req(filtered_base())
    df7[df7$Circulating.Library %in% filtered_base()$Owning.Library,]
  })
...
#Output E1: Total items (excluding circ modifier ILL)
  get_totalitems <- function() {
    ...
    if (nrow(fb_filtered) == 0) {
      return(data.frame(
        Owning.Library = character(0),
        Circulating.Library = character(0),
        Circulation.Modifier = character(0),
        Count.of.Items = numeric(0)
      ))
    }
    
    data <- aggregate(Count.of.Items ~ Owning.Library + Circulating.Library + Circulation.Modifier,
                      data = fb_filtered, FUN = sum)
    
    grand_total <- data.frame(
      Owning.Library = "Grand Total",
      Circulating.Library = "",
      Circulation.Modifier = "All",
  ...
  makeDownloadHandler("download_totalitems", "totalitems", get_totalitems)
